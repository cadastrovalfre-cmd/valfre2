    ...
    const products = data.Products.map((item) => {
      ...
      if (Array.isArray(p.ProductVariant) && p.ProductVariant.length > 0) {
        variants = p.ProductVariant.map((v) => {
          ...
          return {
            ...
            promotional_price: toNumber(v.promotional_price || p.promotional_price),
            available:
              toBool(v.available) ||
              variantStock > 0,
            image: variantImage,
      ...
      if (Array.isArray(p.ProductStock) && p.ProductStock.length > 0) {
        stock = p.ProductStock.reduce((total, s) => {
          return total + toNumber(s.quantity ?? s.stock ?? s.balance ?? 0);
        }, 0);
      }
      ...
      if (stock === 0 && variants.length > 0) {
        stock = variants.reduce((total, v) => total + toNumber(v.stock), 0);
      }
